...
pub fn build_mesh(&self) -> Result<Mesh<DIM>, crate::error::Error> {
        ...
        self.grid.map_faces(|face| {
            ...
            let (bnd, g1, r1) = if face.cell1.is_none() {
                (Some(face.boundary.unwrap()), false, own_rank)
            } else {
                let c1 = face.cell1.as_ref().unwrap();
                (None, c1.is_ghost, c1.owner_rank)
            };
        ...
        // add patches
        for bnd in 0..(self.grid.boundary_len() as u16) {
            mesh.add_patch(bnd, self.grid.boundary_name(bnd), None).unwrap();
        }
    ...
    for i in 1..faces.len() {
        ...
        let found = if found.is_some() {found.unwrap()} else {
            ...
            for j in 0..faces.len() {
                let p0 = mesh.node(mesh.face(faces[j]).nodes()[0]).position();
                let p1 = mesh.node(mesh.face(faces[j]).nodes()[1]).position();
                all.push((p0, p1));
            }
            panic!("error in reorder_polygon_faces, next node not found for face {} / {}, previous: {:?} {:?}, left: {:?}, all: {:?}", i, faces.len(), n0, n1, left, all);
        };
